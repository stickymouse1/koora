...
<body>
 ...
 <?php 
  ...
  //echo($Id_T."<br>");
    ...
    $Nom_Cp=addslashes($_POST['Nom_Cp']);
 // echo($Nom_Cp."<br>");
   $Date_D_Pp=addslashes($_POST['Date_D_Pp']);
 // echo($Date_D_Pp."<br>");
    $Date_F_Cp=addslashes($_POST['Date_F_Cp']);
  //echo($Date_F_Cp."<br>");                                                  

  //echo($Poids_CI."<br>");  
  

   $Type_Cp=addslashes($_POST['Type_Cp']);

 // echo($Email_Arch."<br>");  
   $fichier = basename($_FILES['Affiche_Cp']['name']);
  ...
  if(!isset($erreur)) //S'il n'y a pas d'erreur, on upload
{
     ...
     if(move_uploaded_file($_FILES['Affiche_Cp']['tmp_name'], $dossier . $fichier)) //Si la fonction renvoie TRUE, c'est que ça a fonctionné...
     {
  $req="INSERT INTO `competition`(`Id_Cp`, `Date_D_Pp`, `Date_F_Cp`, `Nom_Cp`, `Type_Cp`, `Affiche_Cp`, `Id_T`) VALUES 
   ('','$Date_D_Pp','$Date_F_Cp','$Nom_Cp','$Type_Cp','$fichier','$Id_T')";
  $res=mysql_query($req) or die (mysql_error()) ; 
if($res)
{
	echo("<p align='middle'><strong><font face='Calibri'  size='6' color='#336600'>Ajout de compétition sportive : <strong><font face='Calibri'  size='7' color='#00FF99'> ". $Nom_Cp."   organisé au terrain  : <strong><font face='Calibri'  size='7' color='#00FF99'> ". $Nom_T." </fond></strong><font face='Calibri'  size='6' color='#336600'> effectue avec succés</font></strong><br> ");
		echo("<a href='competition.php'><i><strong><font face='Calibri'  size='6' color='#669999'>Retour</font></strong></i> </a>");
}
else
{
