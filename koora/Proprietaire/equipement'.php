...
<body>
 ...
 <?php 
  ...
  //echo($Id_T."<br>");
    ...
    $Designation_Eq=addslashes($_POST['Designation_Eq']);
 // echo($Designation_Eq."<br>");
   $Categorie_Eq=addslashes($_POST['Categorie_Eq']);
 // echo($Categorie_Eq."<br>");
    $Prix_Eq=addslashes($_POST['Prix_Eq']);
  //echo($Prix_Eq."<br>");                                                  

  //echo($Poids_CI."<br>");  
  

   $Description_Eq=addslashes($_POST['Description_Eq']);

 // echo($Email_Arch."<br>");  
   $fichier = basename($_FILES['Photo_Eq']['name']);

			$dossier='../uploadEq/';		
  require_once'../connexion.php'; 
    $req="SELECT * FROM `terrain` where  `terrain`.`Id_T` ='$Id_T' ";
  ...
  if(!isset($erreur)) //S'il n'y a pas d'erreur, on upload
{
     ...
     if(move_uploaded_file($_FILES['Photo_Eq']['tmp_name'], $dossier . $fichier)) //Si la fonction renvoie TRUE, c'est que ça a fonctionné...
     {
  $req="INSERT INTO `equipement`(`Ref_Eq`, `Designation_Eq`, `Categorie_Eq`, `Prix_Eq`, `Description_Eq`, `Photo_Eq`, `Id_T`) VALUES
   ('','$Designation_Eq','$Categorie_Eq','$Prix_Eq','$Description_Eq','$fichier','$Id_T')";
  $res=mysql_query($req) or die (mysql_error()) ; 
if($res)
{
	echo("<p align='middle'><strong><font face='Calibri'  size='6' color='#336600'>Ajout d'équipement sportif : <strong><font face='Calibri'  size='7' color='#00FF99'> ". $Designation_Eq."   au terrain  : <strong><font face='Calibri'  size='7' color='#00FF99'> ". $Nom_T." </fond></strong><font face='Calibri'  size='6' color='#336600'> effectue avec succés</font></strong><br> ");
		echo("<a href='equipement.php'><i><strong><font face='Calibri'  size='6' color='#669999'>Retour</font></strong></i> </a>");
}
else
{
