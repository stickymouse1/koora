...
<body>
 ...
 <?php 
  $Nom_Eq=addslashes($_POST['Nom_Eq']);
  //echo($Nom_Eq."<br>");
  
    $Date_C_Eq=addslashes($_POST['Date_C_Eq']);
 // echo($Date_C_Eq."<br>");
  
   $fichier = basename($_FILES['Logo_Eq']['name']);
  ...
  if(!isset($erreur)) //S'il n'y a pas d'erreur, on upload
{
     ...
     if(move_uploaded_file($_FILES['Logo_Eq']['tmp_name'], $dossier . $fichier)) //Si la fonction renvoie TRUE, c'est que ça a fonctionné...
     {
  $req="INSERT INTO `equipe`(`Id_Eq`, `Nom_Eq`, `Logo_Eq`, `Date_C_Eq`) VALUES   ('','$Nom_Eq','$fichier','$Date_C_Eq')";
  $res=mysql_query($req) or die (mysql_error()) ; 
if($res)
{
		 $q1="SELECT Id_Eq FROM equipe  where 1  ORDER BY `equipe`.`Id_Eq` Desc";
	 $p1= mysql_query ($q1) or die ("Select impossible");
	  $res1=mysql_fetch_array($p1);
		     $Id_Eq=$res1['Id_Eq'];
			 
	 $req2="INSERT INTO `appartient`(`Id_S`, `Id_Eq`) VALUES 
   ('$Id_S','$Id_Eq')";
  $res2=mysql_query($req2) or die (mysql_error()) ; 
	echo("<p align='middle'><strong><font face='Calibri'  size='6' color='#336600'>Ajout d'équipe sportif : <strong><font face='Calibri'  size='7' color='#00FF99'> ". $Nom_Eq."   apar le sportif  : <strong><font face='Calibri'  size='7' color='#00FF99'> ". $Nom_S." ".$Prenom_S." </fond></strong><font face='Calibri'  size='6' color='#336600'> effectue avec succés</font></strong><br> ");
		echo("<a href='equipe.php'><i><strong><font face='Calibri'  size='6' color='#669999'>Retour</font></strong></i> </a>");
}
else
{
