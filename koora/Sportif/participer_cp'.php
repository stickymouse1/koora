...
document.getElementById("msg").innerHTML='<strong><font color="#FF0000">Equipement existe déja</font></strong>';
...
document.getElementById("msg1").innerHTML='<strong><font color="#FF0000"> Pas d equipement selectionné</font></strong>';
}
else
{
...
<body>
  ...
  $Id_Cp=addslashes($_POST['Id_Cp']);
  //echo($Id_Cp."<br>");
  
 $hi_temp = $_POST["hi_temp"];
 // echo($Details_R."<br>");                                                                                              
require_once'../connexion.php'; 
   $req="SELECT * FROM `sportif` where Email_S='$Login' and Pwd_S='$Pwd' ";
...
$Id_S=$enreg['Id_S'];
$Nom_S=$enreg['Nom_S'];
$Prenom_S=$enreg['Prenom_S'];
$Date_N_S=$enreg['Date_N_S'];
$Adr_S=$enreg['Adr_S'];
$Tel_S=$enreg['Tel_S'];
$Email_S=$enreg['Email_S'];
$Pwd_S=$enreg['Pwd_S'];
$Photo_S=$enreg['Photo_S'];
}
 // $Ref_Eq=$_GET['Ref_Eq'];
 //echo( $Ref_P);
  $data =explode("/", $hi_temp);
							   ...
							   if($part)
							   {
							    $requete = "INSERT INTO `jouer`(`Id_Eq`, `Id_Cp`) VALUES ('$part','$Id_Cp')";
...
if($res)
{
	...
	$req="SELECT * FROM `competition`,`terrain` where `terrain`.`Id_T` =`competition`.`Id_T` and `competition`.`Id_Cp`='$Id_Cp'";
$res=mysql_query($req) or die (mysql_error()) ; 
$r=mysql_fetch_array($res);

$Nom_T=$r['Nom_T'];

 $q1="SELECT * FROM competition  where `competition`.`Id_Cp`='$Id_Cp'";
	 $p1= mysql_query ($q1) or die ("Select impossible");
	  $res1=mysql_fetch_array($p1);
		     $Nom_Cp=$res1['Nom_Cp'];
			 $Type_Cp=$res1['Type_Cp'];
$Date_D_Pp=$res1['Date_D_Pp'];
$Date_F_Cp=$res1['Date_F_Cp'];
$Affiche_Cp=$res1['Affiche_Cp'];
	 ...
	 echo("<center><img src='../uploadComp/".$Affiche_Cp."';?> ></center>");
	 echo("<center><h3 class='head1'>Demande de participation au tournoi </h3></center>");
	 echo("<p align='right'><strong><font face='Century Gothic' color='#660000' size='+2'  >Période de joue:</font></strong><font face='verdana' color='#000033' size='+1'> ". $Date_D_Pp." A ".$Date_F_Cp."</font></p>");
	echo ("<p align='left'><strong><font face='Century Gothic' color='#660000' size='+2'  >Compétition: </font></strong><font face='Century Gothic' size='5' color='#000033' >".  $Nom_Cp."</font></p>");
	echo ("<p align='left'><strong><font face='Century Gothic' color='#660000' size='+2'  >Catégorie: </font></strong><font face='Century Gothic' size='5' color='#000033' >".  $Type_Cp."</font></p>");
 echo ("<p align='left'><strong><font face='Century Gothic' color='#660000' size='+2'  >Nom &pr&eacute;nom :</font></strong><font face='Century Gothic' size='5' color='#000033' >".$Nom_S." ".$Prenom_S."</font></p>");
		    ...
		    echo ("<p align='left'><strong><font face='Century Gothic' color='#660000' size='+2'  >Terrain de jeu : </font></strong><font face='Century Gothic' size='5' color='#000033' >".  $Nom_T."</font></p>");
...
$Nom_Eq=$r22['Nom_Eq'];
		...
		echo("<p><center><a href='competitons.php'><font face='Calibri'  size='6' color='#CC0099'>Retour</font> </a></center></p>");
}
else
{
