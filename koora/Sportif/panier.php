...
$Photo_S=$enreg['Photo_S'];
 ...
 //echo( $Ref_P);
 $date1 = date("Y-m-d");
  $req="INSERT INTO `panier`(`Id_P`, `Date_C_P`, `Id_S`) VALUES  ('','$date1','$Id_S')";
...
{
	 $req1=mysql_query("SELECT * FROM `panier` ORDER BY `panier`.`Id_P` DESC");
				  $res1=mysql_fetch_array($req1);
		    $Id_P=$res1['Id_P'];
			 
	
	 $req5="INSERT INTO `regrouper`(`Id_P`, `Ref_Eq`, `Quantite`) VALUES ('$Id_P','$Ref_Eq','1')";
...
if(!$resultat5)
{
	...
	//SELECT * FROM `panier`, `equipement`,`regrouper` where `equipement`.`Ref_P`=`regrouper`.Ref_P and `panier`.`Id_Panier`= `regrouper`.Id_P	
  $req="SELECT * FROM `equipement`,`panier`,`regrouper`,sportif,`terrain` where `sportif`.Id_S=panier.Id_S and `panier`.Id_P=`regrouper`.Id_P and`regrouper`.Ref_Eq=`equipement`.Ref_Eq  and Email_S='$Login' and Pwd_S='$Pwd' and `terrain`.`Id_T` =`equipement`.`Id_T`";
 // $req="SELECT * FROM `equipement`,`terrain` where `terrain`.`Id_T` =`equipement`.`Id_T` ";
$res=mysql_query($req) or die (mysql_error()) ; 
...
?>
         ...
         <div  align="center">
                                 <p> <a href="commande_panier.php" class="register_icon"><i class="ti-arrow-right"></i><i><strong><font face='Trebuchet MS, Arial, Helvetica, sans-serif' size='+1' color='#FF9966'>Passer commande</font></strong></i></a></p>
                                </div>
