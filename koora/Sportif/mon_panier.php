...
$Photo_S=$enreg['Photo_S'];
 ...
 $date1 = date("Y-m-d");
  ...
  $req="SELECT * FROM `panier`,`regrouper`,sportif where `sportif`.Id_S=panier.Id_S and `panier`.Id_P=`regrouper`.Id_P  and Email_S='$Login' and Pwd_S='$Pwd' ";
...
$Ref_Eq=$enreg['Ref_Eq'];
$req1="SELECT * FROM `equipement` where `equipement`.`Ref_Eq` ='$Ref_Eq'";
 // $req="SELECT * FROM `equipement`,`terrain` where `terrain`.`Id_T` =`equipement`.`Id_T` ";
$res1=mysql_query($req1) or die (mysql_error()) ;
$enreg1=mysql_fetch_array($res1);

$Designation_Eq=$enreg['Designation_Eq'];
$Categorie_Eq=$enreg1['Categorie_Eq'];
$Prix_Eq=$enreg1['Prix_Eq'];
$Description_Eq=$enreg1['Description_Eq'];
$Photo_Eq=$enreg1['Photo_Eq'];
$Id_T=$enreg1['Id_T'];
$req2="SELECT * FROM `terrain` where `terrain`.`Id_T`  ='$Id_T'";
 // $req="SELECT * FROM `equipement`,`terrain` where `terrain`.`Id_T` =`equipement`.`Id_T` ";
